    ...
    Remote: Side + 'static,
{
        ...
        Handler: MessageHandler<Local> + 'static,
    {
        ...
        let io_task = {
            ...
            let outgoing_tx_for_io = outgoing_tx.clone();
            async move {
    ...
    async fn handle_io(
        ...
        // Reader and writer run as two concurrent sub-futures. Previously this
        // loop used `select_biased!` with `read_line(...).fuse()`, which creates
        // a fresh `read_line` future on every iteration. If the outgoing branch
        // won a race while `read_line` had already consumed bytes from the
        // `BufReader` into its private accumulator (but hadn't yet seen a `\n`),
        // dropping that future discarded the consumed bytes — `BufReader` had
        // advanced past them, so the next iteration would land mid-line and
        // `serde_json::from_str` would fail with
        // `EOF while parsing a value at line 2 column 0. Raw: ""`.
        //
        // Running the reader as its own long-lived future guarantees
        // `read_line`'s internal state is never dropped mid-message.
        // `future::select` on the two sub-futures still lets either side unblock
        // the other, but without mid-poll cancellation of in-flight line reads.
        let writer_task = writer_loop::<Local, Remote>(outgoing_bytes, outgoing_rx, broadcast.clone());
        let reader_task = reader_loop::<Local, Remote>(
            incoming_bytes,
            incoming_tx,
            outgoing_tx,
            pending_responses,
        ...
        match futures::future::select(writer_task, reader_task).await {
...
async fn writer_loop<Local, Remote>(
...
async fn reader_loop<Local, Remote>(
    incoming_bytes: impl Unpin + AsyncRead,
    incoming_tx: UnboundedSender<IncomingMessage<Local>>,
    outgoing_tx: UnboundedSender<OutgoingMessage<Local, Remote>>,
    pending_responses: Arc<Mutex<HashMap<RequestId, PendingResponse>>>,
    ...
    Remote: Side + 'static,
{
    ...
    loop {
        ...
        match serde_json::from_str::<RawIncomingMessage<'_>>(&incoming_line) {
                ...
                if let Some(id) = message.id {
                    if let Some(method) = message.method {
                        ...
                        match Local::decode_request(&method, message.params) {
                            Ok(request) => {
                                ...
                                {
                                    log::warn!(
                                        "failed to send request to handler, channel full: {e:?}"
                                    );
                                }
                            }
                            Err(error) => {
                                ...
                                if outgoing_tx.unbounded_send(error_response).is_err() {
                                    log::warn!(
                                        "failed to queue error response: writer channel closed"
                                    );
                                }
                            }
                        }
                    ...
                    match Local::decode_notification(&method, message.params) {
                        Ok(notification) => {
                            ...
                            {
                                log::warn!(
                                    "failed to send notification to handler, channel full: {e:?}"
                                );
                            }
                        }
                        Err(err) => {
            ...
            Err(error) => {
                log::error!(
                    "failed to parse incoming message: {error}. Raw: {incoming_line:?}"
                );
            }
        }
    }
